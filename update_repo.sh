...
for name in `find "$REPO" -depth 1 -type d -not -name ".git" -not -name "$REPONAME" -exec basename {} \;`; do
    for name in `find $SRCDIR/$name -type f -not -links 2 -not -name "*.pyo" -not -name ".DS_Store"`; do
        ...
        ERRMSG="Please correct hard links first !"
    done
done
if [ "$ERRMSG" != "" ]; then
	echo "$ERRMSG"
...
for name in `find . -depth 1 -type d -not -name ".git" -exec basename {} \;`; do
    ...
    if [ ! -f "$REPO/$name/$name-$VERSION.zip" ]; then
        ...
        if [ -f "$SRCDIR/$name/addon.xml" ]; then
            cd $SRCDIR
            zip -q -r $REPO/$name/$name-$VERSION.zip $name -x \*.zip -x *build* -x *__pycache__ -x *.externalTool* -x \*.pyo -x \*.pyc -x *.git* -x *.DS_Store* -x *.settings* -x *.project* -x *.pydevproject* -x \*.pxm -x *strings.xml
    	else
            cd $REPO
